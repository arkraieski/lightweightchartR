...
- support common trading-chart patterns like volume overlays, moving averages,
  and indicator panes without a lot of setup

## Why this package?

If you already work with `quantmod`, `xts`, and pipe-based workflows, this
package gives you a more modern interactive charting layer while keeping the
data pipeline familiar.
...
- Shiny support
- A small compatibility bridge for `chartSeries()`-style workflows
...
For users migrating from `quantmod::chartSeries()`, there is also a compact
wrapper:
...
- a small, practical API rather than exposing every charting knob up front

More advanced customization can be added over time where it clearly improves
real workflows.
